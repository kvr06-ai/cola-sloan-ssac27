...
%%%%      the delivery email, for Tim to add back if he wants them.
...
Each mechanism ran on the same 30 simulated leagues for 25 seasons, 6,750 seasons in all, and we report paired differences with 95\% confidence intervals. Simple COLA helps long-drought teams more than the 2019--2026 NBA lottery or 3-2-1 (intervals exclude zero), is at least level with both on parity, and never reads a record, so it offers no reward for losing. Every other mechanism is matched or beaten by Simple COLA or Beckett COLA on every column of Table~1. Every mechanism fell short on parity, however. The longest drought ran near 25 seasons in every arm, and no reduction in never-winners cleared its interval. Parity means no team stays stuck at the bottom year after year. Over 25 seasons every team should get at least one run good enough to win a playoff series; the last two columns of Table~1 count the teams that never did and the longest wait any team endured.
