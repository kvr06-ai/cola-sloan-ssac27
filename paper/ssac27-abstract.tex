...
\textbf{Table 1.} Nine mechanisms on 30 simulated leagues of 25 seasons each, every mechanism on the same 30 starting leagues.\\[2pt]
\emph{Help to long-drought teams}: among the fourteen teams that missed the playoffs, how many draft places earlier a team picks for each additional season since it last won a playoff series (a regression slope; the same clock for every mechanism).\\
\emph{Reward for losing}: take a season as played, drop the fifth-worst team to worst with everything else held, and measure how much its pick improves; zero for any rule that never reads record, and ``losing costs picks'' where losing pushes a team down the order.\\
\emph{Teams never winning a series}: of thirty teams, how many go all 25 seasons without a playoff series win, averaged over leagues.\\
\emph{Longest drought}: the longest stretch any one team goes without a series win.\\[2pt]
