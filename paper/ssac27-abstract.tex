...
All nine mechanisms ran on the same 30 leagues for 25 seasons; we report paired differences with 95\% confidence intervals. Simple COLA helps long-drought teams more than the 2019--2026 NBA lottery or 3-2-1 (intervals exclude zero), and never reads a record, so it offers no reward for losing. On parity, meaning every team gets its turn to win a playoff series, it matches or beats both. Simple COLA or Beckett COLA matches or beats every other mechanism in every column of Table~1. Every mechanism fell short on parity. In every arm some team went nearly 25 seasons without a series win, and no mechanism reduced how many teams never won one by a margin that cleared its interval.
