...
With three objectives in play, no single configuration optimizes all of them. The Pareto frontier identifies the configurations a league office could defensibly choose: those not dominated on every objective by any alternative. Configurations off the frontier are strictly dominated and can be discarded; configurations on the frontier represent legitimate trade-offs the design discussion must weigh.

As a concrete dominance check: Status quo (cid 0) has max-years-between-CF of $28.5$, manipulation gain of $28.57\%$, and rank-1-to-5 spread of $2.77$. Capped \cola{} at $\Max = 150$ (cid 26) has $28.0$, $1.36\%$, and $2.87$ respectively. Capped@150 dominates Status quo on every objective, so Status quo cannot be on the frontier. The same dominance check rules out Classic, Simple, and the 3-2-1 proposal.
